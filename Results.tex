...
for the 8k-core experiments.  In each plot, we gathered data from five
separate 500-cycle experiments.  Most of our measurements show a small
...
\fix{Ron, my hypothesis is that for each one one measurement is either much
  ...
  directory.}  The transfer time of \intransit workflows (Figures
\ref{fig:intransit-extra-xfer-variance} and
...
number of client cores. \fix{Ron, in talking to Nathan, it seems that the
...
send a finish copying the data to a server buffer. \fix{Ron, reword the
  second part of this sentence (after the ``or'').}  The larger wait time on the
