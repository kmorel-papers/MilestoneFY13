...
The \intransit workflow, shown in Figure~\ref{fig:intransit-internal-bar},
...
fragments leaving the domain of the mesh. \fix{Verify that this is true.}
...
\begin{figure}[htbp]
\begin{centering}
\includegraphics[scale=1]{figures/active-blocks-series}
...
We did not have time to run multiple experiments at each data point, but we were
able to complete multiple experiments for the \insitu refined and \intransit
extra workflows.  \fix{Ron, could you reword this?  I'm not sure what it
  is supposed to be saying.  Are you saying that we did not hack up CTH to
  compute the same iteration over and over again and instead ran the
  complete 500 iteration simulation separate each time?  Or are you saying
  that we only have multiple measurements for \insitu refined and
  \intransit extra?  If the latter, I thought you ran each one 5 times, not
  just those.}
\fix{I also have multiple experiments for the other
applications in on Cielo.  I'm not yet ready to downshift that data.}
The plots in Figure~\ref{fig:operation-variance} show the mean and standard
deviation of the important \insitu and \intransit operations
at 10-cycle increments of the 8k run. \fix{Ron, is it really standard
  deviation shown in these plots?  The symbol used looks like a min/max
  range.}  In each plot, we gathered data from five
separate 500-cycle experiments.
...
\caption[Runtime variance of selected operations.]{Plots of the mean time and
standard deviation of important operations at 10-cycle sections of five
500-cycle experiments that uses 8K cores and 1.5m data blocks.}
\label{fig:operation-variance}
\end{centering}
\end{figure*}
