...
done.  These data summarize evaluations run over the course of \fix{Ron,
  find count} core hours of execution.
...
\\ \fix{Can the label be changed to say baseline and refined instead of
  unoptimized and optimized? \ldots yes.  Done. }
...
the experiments that uses 8K processors of the 1.5m block dataset--the ``sweet
...
Figure~\ref{fig:active-blocks}.  \fix{anything we should say about this?}
...
consistent across the different applications is that the time spent performing
...
sytem noise, and software techniques like garbage collection.
Instead of trying to understand the cause of inconsistent behavior in our
experiments, we intend to document the results with the intent of addressing
...
at 10-cycle increments of the 8k run.  In each plot, we gathered data from five
